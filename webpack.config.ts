...
/**
 * 检测内联进 HTML <script> 的 JS 是否含 HTML 实体碰撞。
 *
 * 背景：Terser 可能把变量命名为 lt/gt/amp/quot 等，导致 `&&lt(` 在浏览器
 * 解析 HTML script 时被当作实体 `&lt;` -> `<`，触发 `Unexpected token '<'`
 * 语法错误，整个前端界面白屏。build 后扫描产物，发现碰撞即报错。
 */
const HTML_ENTITY_NAMES = [
  'lt', 'gt', 'amp', 'quot', 'nbsp', 'apos', 'copy', 'reg', 'trade',
  'mdash', 'ndash', 'hellip', 'bull', 'middot', 'para', 'sect', 'deg',
  'plusmn', 'times', 'divide', 'micro', 'cent', 'pound', 'euro', 'yen',
  'dagger', 'permil', 'prime', 'Prime', 'lsaquo', 'rsaquo', 'frasl',
];
function check_html_entity_collision(compiler: webpack.Compiler) {
  // afterEmit 时 asset 可能已被替换成 SizeOnlySource 读不到内容，改用 assetEmitted 直接拿字节
  compiler.hooks.assetEmitted.tap('check_html_entity_collision', (file, { content }) => {
    if (!/\.(html|js)$/.test(file)) return;
    const src = content.toString('utf-8');
    // 只检查 <script> 内联 JS 段；.js 文件整体视为 JS
    ...
    for (const js of segments) {
      for (const ent of HTML_ENTITY_NAMES) {
        // 危险：&&ent( / ||ent( / <ent( / >ent( 且 ent 后非字母数字（避免误报 lts）
        const re = new RegExp(`[&|<>]${ent}\\b`);
        const hit = re.exec(js);
        if (hit) {
          const pos = hit.index;
          throw new Error(
            `\n[check_html_entity_collision] ${file} 检测到 HTML 实体碰撞: ` +
            `"${hit[0]}" (变量名 ${ent} 前有 &/|/<//> 字符)\n` +
            `上下文: ${js.slice(Math.max(0, pos - 40), pos + 40)}\n` +
            `Terser 把某变量命名为 "${ent}"，与 HTML 实体 &${ent}; 冲突，` +
            `会导致浏览器解析 script 时报 "Unexpected token '<'" 白屏。`,
          );
        }
      }
    }
  ...
  return (_env, argv) => ({
      ...
      rules: [
        {
          ...
          use: {
            ...
            options: {
              compilerOptions: {
                isCustomElement: (tag: string) => tag.startsWith('daoyuan-'),
              },
            },
          },
    ...
    plugins: (entry.html === undefined
      ...
      .concat(
        ...
        { apply: schema_dump },
        { apply: tavern_sync },
        { apply: check_html_entity_collision },
        new VueLoaderPlugin(),
        unpluginAutoImport({
          dts: true,
          dtsMode: 'overwrite',
        ...
        argv.mode === 'production'
          ? new TerserPlugin({
              terserOptions: {
                ...
                // reserved 除全局注入变量外，还包含 HTML 实体名，防止产物出现 &&lt( 等
                // 被酒馆/浏览器 HTML 管线解码成 &< 导致 "Unexpected token '<'" 白屏
                mangle: { reserved: ['_', 'toastr', 'YAML', '$', 'z', ...HTML_ENTITY_NAMES] },
              },
            })
          : new TerserPlugin({
