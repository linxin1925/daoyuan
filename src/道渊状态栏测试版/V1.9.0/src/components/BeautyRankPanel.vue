...
async function sendReply(name: string) {
  ...
  try {
    ...
    const reply = await callForumGenerateReply(inject, text, settings);
...
@media (max-width: 520px) {
  ...
  /* 立绘缩略图浮动，仙姿/群芳谱等长文字环绕填进图下方空白，省行高 */
  .rank-item { display: block; }
  .rank-thumb { float: left; margin: 0 0.6em 0.4em 0; }
  .rank-main { display: block; width: auto; overflow: visible; flex: none; }
}
