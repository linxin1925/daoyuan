...
export function buildForumInjectContent(
  ...
  inject += `(请你以另一位匿名道友的身份，针对上一条评论进行互喷/抬杠/吐槽，语气像修仙界坊市泼皮骂街。50字以内，禁止使用<!-- -->或任何思考标签。只输出纯评论内容，不要角色名、不要引号、不要任何格式标记。)`;
...
export async function callForumGenerateReply(
  ...
  // 酒馆原生 generate
  const result = await generate({ user_input: injectContent });
