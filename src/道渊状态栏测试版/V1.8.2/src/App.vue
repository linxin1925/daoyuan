...
const tabs = computed<Tab[]>(() => {
  ...
  return [
    { key: 'gongfa', label: '功法', icon: 'scroll', show: on('gongfa'), badge: _.size(d.主角.功法) || undefined },
    { key: 'craft', label: '副业', icon: 'flask', show: on('craft') },
    { key: 'npc', label: '人物', icon: 'person', show: on('npc'), badge: _.size(d.人物) || undefined },
    { key: 'pet', label: '灵宠', icon: 'spirit', show: on('pet'), badge: _.size(d.灵宠) || undefined },
    { key: 'daolv', label: '道侣', icon: 'crane', show: on('daolv'), badge: _.size(d.道侣) || undefined },
    { key: 'opp', label: '机遇', icon: 'door', show: on('opp'), badge: _.size(d.机遇) || undefined },
    { key: 'beauty', label: '绝色榜', icon: 'flower', show: on('beauty'), badge: _.size(d.绝色榜) || undefined },
    { key: 'trends', label: '动向', icon: 'wind', show: on('trends'), badge: _.size(d.世界.动向) || undefined },
    { key: 'yujian', label: '玉简', icon: 'scroll', show: on('yujian'), badge: _.size(d.玉简) || undefined },
    { key: 'inv', label: '储物', icon: 'bag', show: on('inv'), badge: _.size(d.主角.储物袋) || undefined },
    { key: 'artifacts', label: '器物', icon: 'gem', show: on('artifacts'), badge: _.size(d.主角.器物) || undefined },
    { key: 'map', label: '地图', icon: 'mountain', show: on('map') },
  ];
});
